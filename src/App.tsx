...
const FORM_EMBED_URL =
  "https://docs.google.com/forms/d/e/1FAIpQLSekFP0-8ZBZ6vjG12am9cmWIPAhYWaG3YQyDdz92VVXT4eS6Q/viewform?embedded=true";
// ────────────────────────────────────────────────────────────────────────────
...
export default function App() {
  ...
  // ── NEW: active tab ────────────────────────────────────────────────────────
  const [activeTab, setActiveTab] = useState<"mapa" | "avaliacao">("mapa");
  // ──────────────────────────────────────────────────────────────────────────
  ...
  useEffect(() => {
    ...
    if (savedComments) {
      try { setComments(JSON.parse(savedComments)); } catch (e) { setComments([]); }
    } else {
    ...
    if (savedInteracts) {
      try { setInteractions(JSON.parse(savedInteracts)); } catch (e) { setInteractions({}); }
    } else {
      const initialInteractions: Record<string, TableInteraction> = {};
      INITIAL_GROUPS.forEach((g) => {
        initialInteractions[g.id] = { groupId: g.id, claps: 0, visits: 0 };
      });
  ...
  useEffect(() => {
    if (groups.length > 0) localStorage.setItem("mostra_cientifica_groups", JSON.stringify(groups));
  }, [groups]);
  useEffect(() => {
  ...
  const handleSearchResultClick = (group: ProjectGroup) => {
    const isQuadra1 =
      (group.classCode.startsWith("EMERE01") || group.classCode.startsWith("EMERE02")) &&
      !(group.groupNumber >= 31 && group.groupNumber <= 44);
  ...
  const applyLayoutPreset = (preset: "grid" | "circle" | "rows") => {
    ...
    if (preset === "grid") {
      ...
      const centerX = 44, centerY = 40, radiusX = 28, radiusY = 22;
      classGroups.forEach((group, idx) => {
  ...
  const resetAllInteractions = () => {
    if (confirm("Tem certeza que deseja zerar TODAS as estatísticas de curtidas e visitas?")) {
      ...
      groups.forEach((g) => { zeroed[g.id] = { groupId: g.id, claps: 0, visits: 0 }; });
  ...
  const handleAddClap = (groupId: string) => {
    setInteractions((prev) => {
      ...
      return { ...prev, [groupId]: { ...current, claps: current.claps + 1 } };
    });
  };
  ...
  const handleRegisterVisit = (groupId: string) => {
    setInteractions((prev) => {
      const current = prev[groupId] || { groupId, claps: 0, visits: 0 };
      return { ...prev, [groupId]: { ...current, visits: current.visits + 1 } };
    });
  ...
  const isGroupInQuadra = (group: ProjectGroup, quadra: "Quadra 1" | "Quadra 2") => {
    ...
    if (quadra === "Quadra 1") {
      return (code.startsWith("EMERE01") || code.startsWith("EMERE02")) &&
        !(group.groupNumber >= 31 && group.groupNumber <= 44);
    } else {
  ...
  const classStats = useMemo(() => {
    let totalClaps = 0, totalVisits = 0;
    ...
    currentClassGroups.forEach((g) => {
      ...
      groupClapsMap.push({ title: g.title, count: interaction.claps, mNum: g.groupNumber });
    });
    ...
    return { totalClaps, totalVisits, totalComments: totalCommentsInClass, topApplauded };
  ...
  const handleShare = async () => {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(shareUrl)
        ...
        .catch(() => legacyCopyFallback(shareUrl));
    } else {
  ...
  const legacyCopyFallback = (text: string) => {
    ...
    try {
      ...
      if (successful) setSuccessToast("Link público copiado com sucesso!");
      else throw new Error("Unable to copy");
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-slate-50/70 text-slate-900 font-sans antialiased p-4 sm:p-6 md:p-8">
          ...
          {activeTab === "avaliacao" && (
            ...
            >
              ...
              <div className="bg-indigo-600 px-6 py-5 flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-white font-bold text-sm">Avaliação Geral — Sábado 27/06</h2>
                  <p className="text-indigo-200 text-[11px] font-medium">
                    FIC CAMAR 2026 · Responda o formulário abaixo
                  </p>
                </div>
              </div>
              ...
              <div className="w-full">
                <iframe
                  ...
                  marginHeight={0}
                  marginWidth={0}
                  title="FIC CAMAR 2026 - Avaliação Geral (Sábado 27/06)"
                  className="block"
                  loading="lazy"
                >
              ...
              <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-xs flex flex-col lg:flex-row items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-3 overflow-x-auto w-full lg:w-auto pb-1 lg:pb-0 scrollbar-none">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => { setCurrentClass("TODAS"); setSelectedGroupId(null); }}
                    ...
                    {CLASSES_LIST.filter((c) => {
                      ...
                      return (
                        <button
                          key={c.code}
                          onClick={() => { setCurrentClass(c.code); setSelectedGroupId(null); }}
                          className={`px-4 py-2 rounded-xl text-xs font-bold transition-all whitespace-nowrap active:scale-95 ${
                            isSelected
              ...
              {isAdminMode && (
                <div className="bg-slate-900 text-white rounded-3xl p-5 shadow-lg flex flex-col md:flex-row items-center justify-between gap-4 border border-slate-800 animate-in fade-in duration-300">
                  ...
                  <div className="flex flex-wrap items-center gap-2 bg-slate-800/60 p-2 rounded-2xl border border-slate-700/50">
                    ...
                    <button onClick={() => applyLayoutPreset("grid")} className="flex items-center gap-1 bg-slate-800 hover:bg-slate-700 text-white border border-slate-700/50 text-[10px] font-bold py-1.5 px-3 rounded-xl active:scale-95 transition-all">
                      <Grid className="h-3.5 w-3.5 text-indigo-400" /> Grade
                    </button>
                    <button onClick={() => applyLayoutPreset("circle")} className="flex items-center gap-1 bg-slate-800 hover:bg-slate-700 text-white border border-slate-700/50 text-[10px] font-bold py-1.5 px-3 rounded-xl active:scale-95 transition-all">
                      <Circle className="h-3.5 w-3.5 text-indigo-400" /> Arena
                    </button>
                    <button onClick={() => applyLayoutPreset("rows")} className="flex items-center gap-1 bg-slate-800 hover:bg-slate-700 text-white border border-slate-700/50 text-[10px] font-bold py-1.5 px-3 rounded-xl active:scale-95 transition-all">
                      <Layers className="h-3.5 w-3.5 text-indigo-400" /> Fileiras
                    </button>
                    <div className="h-4 w-[1px] bg-slate-700 mx-1" />
                    <button onClick={handleExportConfig} className="flex items-center gap-1 bg-slate-800 hover:bg-indigo-900/50 hover:text-indigo-300 text-white border border-slate-700/50 text-[10px] font-bold py-1.5 px-3 rounded-xl active:scale-95 transition-all">
                      <Download className="h-3.5 w-3.5" /> Exportar
                    </button>
                    <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-1 bg-slate-800 hover:bg-indigo-900/50 hover:text-indigo-300 text-white border border-slate-700/50 text-[10px] font-bold py-1.5 px-3 rounded-xl active:scale-95 transition-all">
                      <Upload className="h-3.5 w-3.5" /> Importar
                    </button>
                    <input type="file" ref={fileInputRef} onChange={handleImportConfig} accept=".json" className="hidden" />
                    <button onClick={resetAllInteractions} className="flex items-center gap-1 bg-amber-950/40 hover:bg-amber-900/40 text-amber-300 border border-amber-900/50 text-[10px] font-bold py-1.5 px-3 rounded-xl active:scale-95 transition-all">
                      <Trash2 className="h-3.5 w-3.5" /> Zerar Estatísticas
                    </button>
                    <button onClick={resetToInitialLayout} className="flex items-center gap-1 bg-red-950/40 hover:bg-red-900/40 text-red-300 border border-red-900/50 text-[10px] font-bold py-1.5 px-3 rounded-xl active:scale-95 transition-all">
                ...
                <div className="lg:col-span-1 flex flex-col h-full">
                  <div className="flex flex-col gap-4 h-full">
                    <div className="bg-slate-900 rounded-3xl p-6 text-white shadow-xl flex flex-col justify-between relative overflow-hidden">
                      <div className="absolute top-0 right-0 opacity-10 font-black text-6xl select-none pointer-events-none pr-4 pt-2 text-slate-400">📈</div>
                      <div>
                    ...
                    <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex-1 flex flex-col justify-between">
                      <div>
                        <h3 className="text-xs font-black text-slate-900 tracking-tight mb-3 flex items-center gap-1.5 uppercase">
                          ...
                          {currentClass === "TODAS" ? "Destaques da turma" : `Destaques da Turma ${currentClass}`}
                        </h3>
                        <div className="space-y-2.5">
                          {classStats.topApplauded.length > 0 ? (
                            classStats.topApplauded.map((item, idx) => (
                              <div key={idx} className="flex items-center gap-3 p-2.5 rounded-xl border border-slate-100 hover:border-indigo-150 bg-slate-50/50 hover:bg-slate-50 transition-all text-xs">
                                <div className="h-6 w-6 rounded-lg bg-indigo-50 text-indigo-700 border border-indigo-100 flex items-center justify-center font-bold flex-shrink-0">{idx + 1}</div>
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-slate-500 line-clamp-1 font-semibold uppercase text-[10px] tracking-tight">{item.title}</p>
                                </div>
                              </div>
                            ))
                          ) : (
      ...
      <AnimatePresence>
        {selectedGroup && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0" onClick={() => setSelectedGroupId(null)} />
            <motion.div
      ...
      <AnimatePresence>
        {isShareModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0" onClick={() => setIsShareModalOpen(false)} />
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
            ...
            >
              <div className="bg-slate-900 text-white p-5 relative overflow-hidden flex items-center justify-between border-b border-slate-800">
                ...
                <button onClick={() => setIsShareModalOpen(false)} className="z-10 p-1.5 rounded-full hover:bg-white/10 text-slate-300 hover:text-white transition-all cursor-pointer">
                  <X className="h-5 w-5" />
                </button>
              </div>
              <div className="p-6 space-y-5 overflow-y-auto max-h-[80vh]">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 p-3 bg-slate-50 border border-slate-200 rounded-2xl">
                    ...
                    <button
                      onClick={() => { navigator.clipboard.writeText("https://camar-sites.github.io/FIC-CAMAR-2026/"); setSuccessToast("Copiado com sucesso!"); setTimeout(() => setSuccessToast(null), 2500); }}
                ...
                <div className="p-4 bg-emerald-50 rounded-2xl border border-emerald-200/50 space-y-2.5">
                  ...
                  <p className="text-xs text-emerald-950 leading-relaxed">O mapa interativo está agora publicado oficialmente no <strong>GitHub Pages</strong>!</p>
                  <div className="bg-white/80 border border-emerald-150 p-3 rounded-xl space-y-1.5 text-[11px] text-emerald-900 font-medium">
                    <p className="flex items-start gap-1.5"><span className="text-emerald-600 font-extrabold shrink-0">✓</span><span><strong>Sem tela de login:</strong> Qualquer pai, visitante ou aluno pode acessar diretamente pelo celular.</span></p>
                    <p className="flex items-start gap-1.5"><span className="text-emerald-600 font-extrabold shrink-0">✓</span><span><strong>Conexão Segura (HTTPS):</strong> Carrega instantaneamente sem avisos de segurança.</span></p>
                  </div>
                </div>
                <button onClick={() => setIsShareModalOpen(false)} className="w-full py-3 bg-slate-900 hover:bg-slate-850 text-white font-bold text-xs rounded-2xl transition-all active:scale-99 cursor-pointer">
                  Fechar
                </button>
              </div>
            </motion.div>
      ...
      <footer className="max-w-7xl mx-auto mt-12 py-8 text-center text-xs text-slate-400 font-medium border-t border-slate-200">
        <p>© 2026 Feira de Iniciação Científica - FIC CAMAR 2026. Todos os direitos reservados.</p>
